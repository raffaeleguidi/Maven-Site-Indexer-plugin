...
public class Indexer {
    ...
    private Integer nbSeparator(String filename) {
        String relative = relativeToStart(filename);
        return StringUtils.countMatches(relative, File.separator);
    }
    ...
    private void addTags(String filename) {
        try {
            ...
            if (oldText.indexOf("id=\"searchbox-frame\"") > 0) {
                log.info("tags already added to '" + relativeToStart(filename) + "', quitting");
                return;
            }

            log.info("applying tags to '" + relativeToStart(filename) + "'...");

            String relativePoints = "";
            for (int i = 0; i < nbSeparator(filename); i++ ) {
                relativePoints = relativePoints + "../";
            }

            String newText = oldText.replaceAll("</body>",
                    "<div id=\"searchbox\">" +
                            "  <iframe id=\"searchbox-frame\" src=\"" + relativePoints + "searchbox.html\" width=\"100%\" style=\"border: 0\" height=\"100%\">" +
                            "  </iframe>" +
                            "</div>" +
                            signature +
                            "</body>"
            ...
            writer.close();
            log.info("applied tags to '" + relativeToStart(filename) + "'");
    ...
    private void crawlFolder(String dirName, FileOutputStream out) throws IOException {
        ...
        if (files == null) {
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                if (!"searchbox.html".equals(filename)) {
                    ...
                    addTags(dir.getAbsolutePath() + File.separator + filename);
                }
            }
        }
        ...
        if (dirs == null) {
            ...
            for (int i = 0; i < dirs.length; i++) {
                ...
                crawlFolder(dir + File.separator + subDirName, out);
            }
        }
